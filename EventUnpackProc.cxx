...
std::valarray<Long64_t>  BM_S2_Tdiffs(BM_S2_MaxTdiffs); 		// saves S2 time differences from get_BM_LDiff_S2 for online analysis
const Int_t BM_S4_MaxTdiffs = 500;
//const Int_t BM_S4_MaxTdiffs = 100000; // if 1kHz is expected at sc41
std::valarray<Long64_t>  BM_S4_Tdiffs(BM_S4_MaxTdiffs); 		// saves S4 time differences from get_BM_LDiff_S4 for online analysis
    ...
    //GeID;
     ...
     /**------------------Germanium Raw Energy -----------------------------------------**/
      Germanium_hits = RAW->get_Germanium_am_Fired();
       ...
       //  cout<<"RAW->get_Germanium_Trace_Length() " <<RAW->get_Germanium_Trace_Length() << endl;
          if(Germanium_TRACES_ACTIVE){
	...
	for(Int_t i=0; i<BM_Hits; ++i) {
		...
		if(BM_S2_count % BM_S2_DoAnalysisEvery == 0) { // analysis of Tdiff data every BM_S2_DoAnalysisEvery number of hits
			...
			for(Int_t k=0; k<BM_S2_MaxTdiffs; ++k) {
				if((Double_t) BM_S2_SumTdiff < (Double_t) BM_NTimeMax*pow(10,4)) {
					BM_S2_SumTdiff += BM_S2_Tdiffs[ ( BM_S2_count + k ) % BM_S2_MaxTdiffs ];
					//cout << BM_S2_Tdiffs[ ( BM_S2_count + k ) % BM_S2_MaxTdiffs ] << endl;
					hBM_s2h_t1->Fill((Double_t) BM_S2_SumTdiff*pow(10,-4));
					} 
				else {
			...
			for(Int_t j=0; j<BM_MaxTimeDiff; ++j) { 
	...
	for(Int_t i=0; i<BM_Hits; ++i) {
		...
		if(BM_S4_count % BM_S4_DoAnalysisEvery == 0) { // analysis of Tdiff data every BM_S4_DoAnalysisEvery number of hits
			...
			for(Int_t k=0; k<BM_S4_MaxTdiffs; ++k) {
				if((Double_t) BM_S4_SumTdiff < (Double_t) BM_NTimeMax*pow(10,4)) {
					BM_S4_SumTdiff += BM_S4_Tdiffs[ ( BM_S4_count + k ) % BM_S4_MaxTdiffs ];
					hBM_s4h_t1->Fill((Double_t) BM_S4_SumTdiff*pow(10,-4));
					} 
				else {
